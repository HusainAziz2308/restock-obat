                ...
                <div class="bg-white rounded-2xl shadow-sm border border-slate-100 overflow-hidden hover:shadow-xl hover:-translate-y-1 transition-all duration-300 flex flex-col justify-between">
                    <div>
                        <div class="relative h-64 bg-slate-900 overflow-hidden flex items-center justify-center">
                            ...
                            @endif
                            <div class="absolute bottom-3 left-3 bg-blue-600 text-white text-xs font-bold px-3 py-1 rounded-full shadow-md">
                                NIM: {{ $mhs['nim'] }}
                            </div>
                        </div>
                        <div class="p-6">
                            <h3 class="text-xl font-bold text-slate-900 mb-1 tracking-tight hover:text-blue-600 transition-colors">
                            ...
                            <div class="space-y-2">
                                <span class="text-xs font-semibold text-slate-400 uppercase tracking-wider block">Keahlian:</span>
                                <div class="flex flex-wrap gap-1.5">
                                    @foreach ($mhs['keahlian'] as $skill)
                                    <span class="bg-blue-50 text-blue-600 text-xs font-medium px-2.5 py-1 rounded-md border border-blue-100">
                                        {{ $skill }}
                                    </span>
                                    @endforeach
                                </div>
                    ...
                    <div class="p-6 pt-0 mt-4">
                        <a href="{{ url('/about/' . $mhs['nim']) }}" class="w-full inline-flex items-center justify-center px-4 py-2.5 border border-slate-200 text-sm font-semibold text-slate-700 bg-white hover:bg-slate-50 hover:text-slate-900 rounded-xl transition-all duration-200 gap-2 group">
                            Lihat Profil Detail
                        </a>
                    </div>
                </div>
