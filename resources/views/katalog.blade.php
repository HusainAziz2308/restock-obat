...
@section('page_description',
    'Tingkatkan efisiensi inventaris farmasi Anda dengan sistem manajemen gudang cerdas kami.
        ...
        <div class="max-w-full mx-auto px-6 md:px-12">
            ...
            <div class="text-center mb-16">
                ...
                <p class="text-slate-500 max-w-2xl mx-auto">
                    Tingkatkan efisiensi inventaris farmasi Anda dengan sistem manajemen gudang cerdas kami. Tidak ada biaya
            ...
            <div class="flex flex-col md:flex-row justify-center gap-8 max-w-7xl mx-auto items-center md:items-stretch">
                @foreach ($packages as $package)
                    <div
                        class="{{ $package['is_featured'] ? 'bg-blue-600 rounded-3xl p-8 border border-blue-600 shadow-2xl transform md:-translate-y-5 hover:-translate-y-10 transition-all duration-300 relative' : 'bg-white rounded-3xl p-8 border border-slate-200 shadow-sm hover:shadow-xl hover:-translate-y-5 transition-all duration-300' }}">

                        <div class="absolute top-0 right-8 transform -translate-y-1/2 flex flex-col gap-1 items-end">
                            @if ($package['badge'])
                                <span
                                    class="bg-amber-400 text-amber-900 text-xs font-bold px-3 py-1 rounded-full uppercase tracking-wide shadow-sm">
                                    {{ $package['badge'] }}
                                </span>
                            @endif
                        ...
                        <div class="mb-6 {{ $package['is_featured'] ? 'text-white' : 'text-slate-900' }}">
                            <div class="flex items-center gap-2 mb-1">
                                ...
                                @if (isset($package['discount']) && $package['discount'])
                                    <span
                                        class="bg-red-500 text-white text-[10px] font-bold px-2 py-0.5 rounded uppercase tracking-wide shadow-sm">
                                        {{ $package['discount'] }}
                                    </span>
                                @endif
                            </div>
                            ...
                            <span
                                class="{{ $package['is_featured'] ? 'text-blue-200' : 'text-slate-500' }}">{{ $package['period'] }}</span>
                        </div>

                        <ul
                            class="space-y-4 mb-8 text-sm {{ $package['is_featured'] ? 'text-blue-50' : 'text-slate-600' }}">
                            @foreach ($package['features'] as $feature)
                                <li class="flex items-center {{ !$feature['available'] ? 'text-slate-400' : '' }}">
                                    @if ($feature['available'])
                                        <svg class="w-5 h-5 mr-3 shrink-0 {{ $package['is_featured'] ? 'text-blue-200' : 'text-emerald-500' }}"
                                            fill="none" stroke="currentColor" viewBox="0 0 24 24">
                                            <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2"
                                                d="M5 13l4 4L19 7"></path>
                                        </svg>
                                    @else
                                        <svg class="w-5 h-5 mr-3 shrink-0 text-slate-400" fill="none"
